  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (scene === 'walk') {
        ...
        const progress = Math.min(leg / 8000, 1)
        ...
        lift = walking ? Math.sin((leg / 320) * Math.PI) ** 2 * 1.5 : 0
      } else if (scene === 'rope') {
        ...
        tilt = cycle < 3200 ? Math.sin(cycle / 180) * 0.35 : 0
      }
    ...
    <div aria-hidden="true" className={`mascot-scene mascot-scene-${scene}`} ref={ref}>
      ...
      <div className="mascot-traveler" ref={travelerRef}>
        <div
          ...
          ref={spriteRef}
          style={{ backgroundImage: `url(/images/mascot/nicolas-sprites.png)` }}
        />
      </div>
